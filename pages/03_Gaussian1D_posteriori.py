...
sigma_prior = st.slider(
    r"Prior position confidence $\sigma_0$", 0.1, 10.0, 1.0, step=0.1
)

st.header(
    r"Observation $y$ and Observation Probability $P(y|x)=\mathcal{N}(y|x, \nu^2)$ (Likelihood $L_y(x)$)"
)
mu_likelihood = st.slider(r"Observation $Y=y$", -10.0, 10.0, 2.0, step=0.1)
sigma_likelihood = st.slider(
    "Likelihood Standard Deviation (σ_likelihood)", 0.1, 10.0, 1.0, step=0.1
)
...
mu_posterior, sigma_posterior = compute_posterior(
    mu_prior, sigma_prior, mu_likelihood, sigma_likelihood
)
...
table_data = {
    ...
    "Formula": [r"$\mu_0$", r"$\sigma_0$", r"$y$", r"$\nu$", r"$\mu_1$", r"$\sigma_1$"],
}
