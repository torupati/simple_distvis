...
if data is not None:
    ...
    n_clusters = st.number_input("Number of clusters", min_value=1, max_value=10, value=2, step=1)
    ...
    if st.button("Run K-means Clustering(scikit-learn)"):
        ...
        scatter = ax2.scatter(data[:, 0], data[:, 1], c=labels, cmap='tab10', alpha=0.7, label="Data")
        ax2.scatter(centers[:, 0], centers[:, 1], color='red', marker='x', s=150, label="Centers")
