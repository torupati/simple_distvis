...
if data is not None:
    ...
    if st.button("Show Mean and Covariance"):
        ...
        plot_cov_ellipse(mean, cov, ax, n_std=2, edgecolor='red', facecolor='none', lw=2, label="Covariance (2σ)")
        ax.legend()

    st.pyplot(fig)

    # GMM EM algorithm
    n_mixtures = st.number_input("Number of Gaussian", min_value=1, max_value=10, value=2, step=1)
    ...
    if st.button("Run GMM Training (in-house)"):
        ...
        gmm.Mu = mu_init
        gmmparam, loglikelihood_history = train_gmm(gmm, data, max_it=20, plot_ckpt=False)
        print(gmmparam)
    ...
    if st.button("Run GMM Clustering (scikit-learn)"):
        ...
        gmm = GaussianMixture(n_components=n_mixtures, covariance_type='full', random_state=0)
        labels = gmm.fit_predict(data[:, :2])
        centers = gmm.means_

        fig4, ax4 = plt.subplots()
        scatter = ax4.scatter(data[:, 0], data[:, 1], c=labels, cmap='tab10', alpha=0.7, label="Data")
        ax4.scatter(centers[:, 0], centers[:, 1], color='red', marker='x', s=150, label="Centers")
